...
class Game():
    ...
    def make_move(self, strikes, enemy, display, player):
        ...
        for i in strikes:
            ...
            if ' # ' in q:
                ...
                display.board_numbers.pop(x)
                display.board_numbers.insert(x, ' B ')

            elif ' ^ ' in q:
                print(f'{state} SUBMARINE has been hit {face}')
                display.board_numbers.pop(x)
                display.board_numbers.insert(x, ' S ')

            elif ' & ' in q:
                print(f'{state} CRUISER has been hit {face}')
                display.board_numbers.pop(x)
                display.board_numbers.insert(x, ' C ')

            elif ' * ' in q:
                print(f'{state} DESTROYER has been hit {face}')
                display.board_numbers.pop(x)
                display.board_numbers.insert(x, ' D ')
    ...
    def fleet(
        ...
        # if player has False value the player is a computer        
            ...
            for count, value in enumerate(display.board_numbers):
                if (value == ' B ' 
                            and display.board_numbers.count(' B ') < 5 
                            and num == letter_b):
                    moves.append(count)
                    num += 1
                    letter_b += 1
                            ...
                            and display.board_numbers.count(' C ') < 4 
                            and num == letter_c):
                    moves.append(count)
                    num += 1
                    letter_c += 1
                            ...
                            and display.board_numbers.count(' D ') < 3 
                            and num == letter_d):
                    moves.append(count)
                    num += 1
                    letter_d += 1
                            ...
                            and display.board_numbers.count(' S ') < 2 
                            and num == letter_s):
                    moves.append(count)
                    num += 1
                    letter_s += 1
            ...
            moves = [i for i in moves if (i + 11) <= 121 and (i + 1) <= 121]

            if len(moves) == 1:
                target.append(moves[0] + 1)
                target.append(moves[0] - 1)
                target.append(moves[0] + 11)
                target.append(moves[0] - 11)

            elif len(moves) >= 2:
                if moves[0] % 11 == moves[1] % 11:
                    target.append(moves[num] + 11)
                    target.append(moves[num] - 11)
                    target.append(moves[0] + 11)
                    target.append(moves[0] - 11)

                elif math.floor(moves[0] / 11) == math.floor(moves[1] / 11):
                    target.append(moves[num] + 1)
                    target.append(moves[num] - 1)
                    target.append(moves[0] + 1)
                    target.append(moves[0] - 1)
            ...
            if board.board.count(symbol) > 2:
                for i in range(missile):
                    if target != []:
                        aim = random.choice(target)
                        target.remove(aim)
                        aim = board.board_numbers[aim]
                    else:
                    ...
                    while (board.board_numbers.index(aim) < 10 
                        ...
                        if target != []:
                            ...
                            aim = random.choice(enemy.board_numbers)
                    strikes.append(aim)
